            ...
            <div class="tab-content mt-4" id="nav-tabContent">
                ...
                @foreach ($seminars as $key => $seminar)
                    <div class="tab-pane fade {{ $key == 0 ? 'show active' : '' }}" id="seminar{{ $key }}"
                        ...
                        <table class="table table-responsive table-success table-striped table-hover shadow rounded mt-5" id="data-table">
                            <thead>
                                <tr>
                                    <th>#</th>
                                    <th>name</th>
                            ...
                            <tbody>
                                @foreach ($seminar->leeds as $key => $leed)
                                    <tr>
                                        ...
                                        <td class="">
                                            <form action="{{ route('leeds.destroy', $leed->id) }}" method="POST"
                                                class="m-2">
                                                @csrf
                                                @method('DELETE')
